...
const Product = ({ item, provider, account, dappazon, togglePop }) => {
  ...
  const fetchDetails = async () => {
    ...
    const orders = events.filter(
      (event) => event.args.buyer.toLowerCase() === account.toLowerCase() && event.args.itemId.toString() === item.id.toString()
    )
      ...
      <div className='product__details'>
        ...
        <div className='product__order'>
          ...
          <p>
            Free delivery<br/>
            <strong>
              {new Date(Date.now() + 338749837).toLocaleDateString(undefined, {weekday: 'long', month: 'long', day: 'numeric'})}
            </strong>
          </p>
